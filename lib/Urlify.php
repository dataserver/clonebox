...
class Urlify
{
    ...
    /**
     * convert transliterate text to uppercase, removing any extra white space or special chars not alphanumeric .
     * useful to database search
     * changes, e.g., "João Ninguém Sant'Anna" to "JOAO NINGUEM SANT ANNA"
     * @param string $text The text to return normalized
     * @return string
     */
    public static function normalize_name($str, $only_alpha = true)
    {
        $str = self::downcode($str);                             // transliterate
        if ($only_alpha) {
            $str = preg_replace("/[^A-Za-z[:space:]]/", ' ', $str);  // replace any non alpha into white space
        } else {
            $str = preg_replace("/[^A-Za-z0-9[:space:]]/", ' ', $str);  // replace any non alphanumeric into white space
        }
        
        // $str = strtoupper($str);                                 // convert to upper case
        $str = preg_replace('/\s+/', ' ', $str);                 // remove any double white space
        $str = trim($str);                                       // trim
