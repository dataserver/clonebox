...
switch ($action) {
    case 'file-upload':
        ...
        if (isset($_FILES) && !empty($_FILES)) {
            ...
            $parent_id = $parent_folder->find($parent_id)->id();
            ...
            for ($i = 0; $i < count($file_array); ++$i) {
                ...
                $name = basename($file->sanitizeFilename($file_array[$i]['name']));
                $size = filesize($file_array[$i]['tmp_name']);
    ...
    case 'file-rename':
        ...
        $file = new File();
        if ($file->find($file_id)->setName($name)->isValid()) {
            $json = [
                'method' => 'file.rename',
                'params' => [
                    'name' => $name,
    ...
    case 'file-delete':
        ...
        $result = $file->find($file_id)->delete()->isValid();
        if ($result) {
            $json = [
                'method' => 'file.delete',
                'params' => [
    ...
    case 'file-move':
        ...
        if ($file->find($file_id)->move($folder_id)->isValid()) {
            $json = [
                'method' => 'file.move',
                'params' => [
                    'file_id' => $file_id,
    ...
    case 'folder-rename':
        ...
        $folder = new Folder();
        if ($folder->find($folder_id)->setName($name)->isValid()) {
            $json = [
    ...
    case 'folder-delete':
        $folder = new Folder();
        $result = $folder->find($folder_id)->delete()->isValid();
    ...
    case 'folder-move':
        ...
        if ($folder->find($folder_id)->move($to_folder)->isValid()) {
            $json = [
                'method' => 'folder.move',
                'params' => [
                    'folder' => $folder_id,
